...
- [⚡ v1.18.2 업데이트 하이라이트](#-v1182-업데이트-하이라이트)
- [✨ 주요 기능](#-주요-기능)
  - [📊 지식 품질](#-지식-품질)
  - [🛠️ 유지 관리](#️-유지-관리)
  - [💬 조회 및 피드백](#-조회-및-피드백)
...
v1.18.2는 `customEntityLimit`와 `customConceptLimit` 설정을 드디어 실제로 적용되게 하는 **PATCH 버그 수정**입니다. 이전에는 `extractionGranularity`가 `custom`으로 설정되어 있을 때, 이러한 상한은 프롬프트의 부드러운 힌트로만 적용되어 LLM이 설정된 상한 8에 대해 12~25개 항목을 반환하고 그 모두가 위키 페이지에 기록되었습니다. 기존 수렴 감지기는 두 타입 모두 상한에 도달한 후에야 *추가 배치*를 중단했기 때문에, 일반적인 단일 배치 케이스(대부분의 노트)에서는 절대 발화하지 않았습니다. #120을 마감합니다.

- **🎯 엔티티와 컨셉을 설정된 상한으로 하드 캡.** 모든 배치를 누적한 직후, 페이지 생성 직전에 플러그인이 두 목록을 `customEntityLimit` / `customConceptLimit`로 슬라이스합니다. 추출 순서에서 처음 N개 항목이 유지됩니다. 프롬프트 지시와 수렴 감지기는 보완 메커니즘으로 유지됩니다(LLM을 유도하고 불필요한 추가 배치를 방지). `default` / `1-5` 세분성 모드의 동작 변경은 없습니다.

이 릴리스에는 같은 기간에 머지된 두 가지 커뮤니티 기여도 포함됩니다: 설정 가능한 파일명 대소문자(Issue #111), 재수집 시 태그 보존(Issue #114). 자세한 내용은 CHANGELOG.md를 참조하세요.

**Custom 추출 모드를 사용하는 모든 사용자에게 이 버전으로 업그레이드할 것을 강력히 권장합니다.** 그렇지 않으면 `customEntityLimit` 설정이 효과를 발휘하지 않습니다.
