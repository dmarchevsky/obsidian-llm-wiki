...
v1.18.2 是一个 **PATCH 修复**，终于让 `customEntityLimit` 和 `customConceptLimit` 设置真正生效。此前，当 `extractionGranularity` 设为 `custom` 时，这些上限只是作为软性的 prompt 提示——LLM 经常在配置 cap=8 的情况下返回 12-25 个条目，并且全部都写入了 wiki 页面。已有的 convergence detector 只在两类都达到上限时才停止 *后续批次*，而最常见的单批次场景根本不会触发。关闭 #120。

- **🎯 实体/概念数量硬性截断到你配置的上限。** 在所有批次累加完毕、生成页面之前，插件将 `accumulation.entities` 和 `accumulation.concepts` 都 `slice` 到 `customEntityLimit` / `customConceptLimit`。保留提取顺序中的前 N 项。Prompt 提示和 convergence detector 仍作为补充机制保留（它们引导 LLM 行为、避免不必要的额外批次）。`default` / `1-5` 粒度模式行为完全不变。

本版本还包含同一时段合并的两项社区贡献：可配置文件名大小写（Issue #111）和 re-ingest 时保留 tags（Issue #114）。完整细节参见 CHANGELOG.md。

**强烈建议所有使用 Custom 粒度模式的用户升级到本版本。** 否则你的 `customEntityLimit` 设置毫无效果。
