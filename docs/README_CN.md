...
v1.19.0 是一个 **MINOR 版本**，专注于**摄入质量和成本优化**。新增高级 LLM 参数设置面板（禁用思考开关、温度、重复惩罚），一个新的基于程序化匹配的引用引证检查工具，用于验证 `## Mentions in Source` 中的每条引用是否能在源文件中找到，以及紧凑型 slug 列表注入让 LLM 能按精确路径链接到 wiki 页面。配合 Auto Smart Fix、摄入/lint 期间状态栏镜像、写入路径 sources 归一化等社区贡献。通过 Six-Gate 质量门禁。

- **🔧 高级 LLM 参数设置。** LLM 配置区新增「默认/自定义」模式选择器。默认模式隐藏所有高级参数并保持禁用思考开启（适合大多数用户）。自定义模式显示禁用思考开关、提取温度（0–2）、查询温度（0–2）和重复惩罚（0–2）。`disableThinking` 字段名在 `data.json` 中保留以保持向后兼容。
- **🔍 引用引证检查（Issue #126）。** 新增 `scanQuoteGrounding()` 纯函数（零 token 成本），验证 `## Mentions in Source` 下的每条引用能否在链接的源文件中找到。支持当前的 `"—— [[sources/slug]]"` 格式和历史的裸引用。Tier 1 = 精确子串匹配；Tier 2 = 归一化匹配（忽略大小写、标点）。由 @DocTpoint 贡献。
- **📋 紧凑 slug 列表（Issue #116）。** `buildCompactSlugList()` 将现有 wiki 页面的排序 slug 列表注入到 analyzeSource prompt 中，让 LLM 在创建 `[[links]]` 时使用精确路径，减少因 50 页索引上限导致的死链不匹配。由 @DocTpoint 贡献。
- **🛡️ 纯推理响应检测（Issue #99）。** 当 `disableThinking=true` 但模型返回空 content + 高推理 token 时，抛出可操作错误。对拒绝 `thinking.type='disabled'` 的提供商自动 400 回退到 `chat_template_kwargs`。
- **⚡ Stage 4 无操作跳过（Issue #131 Tier 1）。** Stage 4 LLM 调用减少约 33% ——当源仅包含回退字符串时跳过相关页面更新的 LLM 调用。由 @DocTpoint 贡献。
- **📊 Lint 报告增强。** 摘要现在包含无来源引证和越界标签计数。`lintTagViolationSection` 完成全部 8 种语言的国际化。
- **🔊 摄入/lint 期间状态栏镜像（PR #110）。** 由 @dmarchevsky 贡献。
- **🤖 Auto Smart Fix 设置（PR #109）。** Lint 可自动运行所有修复阶段而不显示报告模态框。
- **📝 写入路径 sources 归一化（PR #127）。** 每次 `createOrUpdateFile()` 调用时运行 `fixPollutedSources()`。由 @DocTpoint 贡献。
- **🧹 启动快速修复 Notice 简化。** 更清晰简洁的布局。移除了易误导的 watchedFolders 调试日志。语言下拉标签现在仅使用原生语言名。

**强烈建议所有用户升级到本版本。** 你也可以在 [CHANGELOG.md](../CHANGELOG.md) 中查看完整的变更列表。
