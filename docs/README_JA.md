...
v1.18.1 は、Obsidian コミュニティプラグインのソースコード審査コンプライアンス問題を解決した **PATCH ホットフィックス** です。v1.18.0 のリリースは、ソースコードに `document`（グローバル変数）と `obsidianmd/prefer-active-doc` を対象とした `eslint-disable` コメントが含まれていたため、審査で拒否されました。本ホットフィックスでは `document` のフォールバックとすべての関連 eslint-disable コメントを削除し、`activeDocument` のスタブをテストセットアップに集中化しました。ユーザーに見える動作の変更はありません。

- **🛡️ Obsidian 審査コンプライアンス。** プロダクションコードは `activeDocument`（Obsidian のポップアウトウィンドウ対応ドキュメント参照）のみを使用します。jsdom テスト環境は `setup.ts` の集中スタブを介して `activeDocument` を受け取り、テストとプロダクションの関心事を明確に分離します。

**すべての v1.18.0 機能は変更なし。** v1.18.0 を既にご利用の場合、このホットフィックスに新機能は含まれません。

**本バージョンへの全ユーザーアップグレードを強く推奨します。** 本リリースにより、プラグインは Obsidian の自動ソースコード審査に合格し、コミュニティプラグイン市場で利用可能になります。
