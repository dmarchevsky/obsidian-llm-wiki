...
v1.18.2 は、`customEntityLimit` と `customConceptLimit` 設定を初めて実際に効かせる **PATCH バグ修正** です。以前は、`extractionGranularity` を `custom` に設定しても、これらの上限はプロンプトへの緩いヒントとしてしか効かず、LLM は設定された上限 8 に対して 12〜25 項目を返し、そのすべてが Wiki ページに書き込まれていました。既存の収束検出器は両タイプが上限に達して初めて*追加バッチ*を停止しただけで、一般的な単一批ケース（ほとんどのノート）では決して発火しませんでした。#120 をクローズ。

- **🎯 エンティティとコンセプトを設定された上限にハードキャップ。** 全バッチを蓄積し、ページ生成直前に、プラグインが両リストを `customEntityLimit` / `customConceptLimit` に切り詰めます。抽出順で先頭の N 項目が保持されます。プロンプト指示と収束検出器は補完的な仕組みとして残ります（LLM を導き、不要な追加バッチを回避）。`default` / `1-5` 粒度モードの動作変更はありません。

このリリースには同時期マージされた 2 つのコミュニティ貢献も含まれます：設定可能なファイル名の大文字小文字（Issue #111）、再取り込み時のタグ保持（Issue #114）。詳細は CHANGELOG.md をご覧ください。

**Custom 抽出モードをご利用の全ユーザーに本バージョンへのアップグレードを強く推奨します。** アップグレードしない場合、`customEntityLimit` 設定は効果を持ちません。
